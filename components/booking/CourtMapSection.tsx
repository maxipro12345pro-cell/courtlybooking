...
export default function CourtMapSection({ date }: { date: string }) {
  ...
  function backToMap() { setSelectedCourtId(null); setSelectedTime(null); setViewMode("map3d"); }
  ...
  async function continueBooking() {
    ...
    sessionStorage.setItem(`booking:${bookingId}`, JSON.stringify({
      id: bookingId,
          ...
          <AnimatePresence initial={false} mode="popLayout">
            {viewMode !== "schedule" ? (
              ...
              <motion.div key="schedule" className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <Schedule court={selectedCourt} selectedTime={selectedTime} onSelectTime={setSelectedTime} onBack={backToMap} />
              </motion.div>
            ) : null}
          </AnimatePresence>
        </div>
        <BookingSummary court={selectedCourt} time={selectedTime} date={date} loading={holding} onContinue={continueBooking} />
      </div>
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-sand bg-white p-4 shadow-[0_-12px_40px_rgba(17,24,39,.12)] xl:hidden"><MobileSummary court={selectedCourt} time={selectedTime} loading={holding} onContinue={continueBooking} /></div>
    </LayoutGroup>
  );
}
